                   ...
                   (\ti -> concat <$> forM caps_of_interest (\cap_name -> do
                        ...
                        return $ case Terminfo.getCapability ti (Terminfo.tiGetStr cap_name) of
                            Just cap_def -> [verify (case_name ++ " -> " ++ show cap_def)
                                                    (verify_parse_cap cap_def $ const succeeded)]
                            Nothing      -> []
                    )
                   )
        )
...
verify_parse_cap cap_string on_parse =
...
non_paramaterized_caps (NonParamCapString cap) = do
    verify_parse_cap cap $ \e -> 
        let expected_bytes = map (toEnum . fromEnum) cap
            out_bytes = bytes_for_range e 0 (length cap)
        in verify_bytes_equal out_bytes expected_bytes

literal_percent_caps (LiteralPercentCap cap_string expected_bytes) = do
    verify_parse_cap cap_string $ \e -> verify_bytes_equal (collect_bytes e) expected_bytes
...
push_param_caps (PushParamCap cap_string expected_param_count expected_bytes) = do
    verify_parse_cap cap_string $ \e ->
        let out_bytes = collect_bytes e
            out_param_count = param_count e
        in if out_param_count == expected_param_count
...
print_expression ti cap_name = do
    let parse_result = parse_cap_expression $ from_capname ti cap_name
