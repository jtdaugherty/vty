...
-- keys and combinations will not reliably map to the expected events
-- by any terminal program. Even those not using vty. There is no 1:1
...
--
-- See "Graphics.Vty.Config" for how to configure vty's input
...
--  2. 8 bit
--
-- 7 bit input is the default and the expected in most use cases. This
-- is what vty uses.
--
...
-- the two high bit wires: 6 and 7. This is why control key combos map
-- to single character events: The input bytes are identical. The input
-- byte is the bit encoding of the character with bits 6 and 7 masked.
-- Bit 6 is set by shift. Bit 6 and 7 are masked by control. EG:
--
-- * Control-I is 'i', `01101001`, has bit 6 and 7 masked to become
-- `00001001`. Which is the ASCII and UTF-8 encoding of the tab key.
--
-- * Control+Shift-C is 'C', `01000011`, with bit 6 and 7 set to zero
-- which makes `0000011` and is the "End of Text" code.
--
...
-- an encoded control key. To differentiate a single @ESC@ eventfrom a
...
-- followed by more bytes read within 'singleEscPeriod'. All bytes up
-- until the next valid input block are passed to the classifier.
--
...
-- no implementation.
--
-- Vty used to emulate @VMIN@ and @VTIME@. This was a input loop
-- which did tricky things with non-blocking reads and timers. The
...
-- entirely and just go directly to scancodes. A reasonable option for
-- vty if everybody used the linux kernel console. I hear GUIs are
-- popular these days. Sadly, GUI terminal emulators don't provide
-- access to scancodes AFAIK.
--
-- All is lost? Not really. "Graphics.Vty.Config" supports customizing
-- the input byte to event mapping and xterm supports customizing the
...
--
-- This isn't the only odd aspect of terminals due to historical
-- aspects that no longer apply. EG: Some terminfo capabilities specify
...
-- the the applicable entries from 'inputMap'.
--
-- The terminal device is configured with the attributes:
--
-- * IXON disabled
--      - disables software flow control on outgoing data. This stops
--        the process from being suspended if the output terminal cannot
--        keep up. I presume this has little effect these days. I hope
--        this means that output will be buffered if the terminal cannot
--        keep up. In the old days the output might of been dropped?
--
