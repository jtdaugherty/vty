...
putAttrChange :: MonadIO m => Output -> InlineM () -> m ()
putAttrChange out c =
    let fattr  = FixedAttr defaultStyleMask Nothing Nothing
        attr   = execState c currentAttr
        attr'  = limitAttrForDisplay out attr
        fattr' = fixDisplayAttr fattr attr'
        diffs  = displayAttrDiffs fattr fattr'
    in outputDisplayCommands out $ do
        -- always reset to default attribute.
...
putAttrChange_ :: MonadIO m => InlineM () -> m ()
